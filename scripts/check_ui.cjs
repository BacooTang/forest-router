...
(async()=>{
 ...
 try{
  ...
  await page.locator('#nav button').filter({hasText:'设置'}).click();await page.locator('#apiKey').fill('changed-company-key');await page.getByRole('button',{name:'保存设置'}).click();await page.waitForTimeout(300);
